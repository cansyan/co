...
func (a *App) pushHistory(path string, pos Pos) {
	if a.navigatingHist {
		return
	}
...
func (sb *SearchBar) updateMatches() {
	...
	for {
		...
		sb.matches = append(sb.matches, Pos{
			Row: lineCount,
			Col: runeCol,
		})
...
func (sb *SearchBar) syncEditor() {
	...
	editor.SetSelection(m, Pos{Row: m.Row, Col: m.Col + queryLen})
}
...
func highlightGo(line []rune) []StyleSpan {
	...
	for i := 0; i < len(line); {
		...
		case stateInString:
			if r == '"' {
				spans = append(spans, StyleSpan{Start: start, End: i + 1, Style: ui.Theme.Syntax.String})
				state = stateDefault
			}
		...
		case stateInComment:
			spans = append(spans, StyleSpan{Start: start, End: len(line), Style: ui.Theme.Syntax.Comment})
			return spans
		}
		i++
	}
...
func highlightMarkdown(line []rune) []StyleSpan {
	var spans []StyleSpan
	if len(line) == 0 {
		return spans
	}
